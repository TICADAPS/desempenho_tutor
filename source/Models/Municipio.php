...
class Municipio extends Model {
    ...
    public function load(int $id, string $columns = "*"): ?Municipio {
        $load = $this->read("SELECT {$columns} FROM " . self::$entity . " WHERE Estado_cod_uf = :id", "id={$id}");
     ...
     * @return null|User
     */
    public function findByIdUf(int $iduf, string $columns = "*"): ?array {
        return $this->find("Estado_cod_uf = :id", "id={$iduf}", $columns);
    }
    
    public function findByUFid($idUf): ?array {
        $allUf = $this->read("SELECT * FROM municipio where Estado_cod_uf = '$idUf'");

        if ($this->fail() || !$allUf->rowCount()) {
            return null;
        }
        return $allUf->fetchAll(\PDO::FETCH_CLASS, __CLASS__);
    }
    ...
    public function municipioPorUf(int $codUf): ?array {
        $all = $this->read("SELECT cod_munc, Municipio, ibge from municipio WHERE Estado_cod_uf = $codUf order by Municipio");
    ...
    public function allInnerJoinMunicipioEstado(): ?array {
        $all = $this->read("SELECT * from municipio inner join estado on municipio.Estado_cod_uf = estado.cod_uf order by municipio.Municipio, estado.UF;");
    ...
    public function findJoinMunicipioEstadoById($codmun): ?array {
        $all = $this->read("SELECT * from municipio inner join estado on municipio.Estado_cod_uf = estado.cod_uf "
                . "where municipio.cod_munc = {$codmun} order by municipio.Municipio, estado.UF;");
