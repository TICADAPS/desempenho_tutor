...
$result_transacoes = "select m2.NomeMedico as tutor ,med.NomeMedico as bolsista,ct.Edicao,ct.PeriodoInicial,ct.PeriodoFinal,med.Convocacao
	from medico med
        inner join ControleCalendario cc on med.idMedico = cc.idMedico
        inner join CalendarioTutoria ct on cc.idCalendario = ct.idCalendario and cc.Edicao = ct.Edicao
        inner join vaga_tutoria vt on vt.idMedico = cc.idMedico
        inner join medico m2 on m2.idMedico = vt.idTutor   
        where cc.Edicao = 1
        order by m2.NomeMedico, ct.PeriodoInicial;";
